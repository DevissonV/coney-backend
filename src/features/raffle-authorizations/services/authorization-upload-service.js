 ...
 * This service validates the file and uploads it to S3.
 * @class AuthorizationUploadService
 */
class AuthorizationUploadService {
  /**
   * Uploads a document file to S3 and returns the public URL.
   *
  ...
  async uploadDocument({ buffer, originalname, raffleId, type }) {
    try {
      ...
      const fileUrl = await uploadPublicFile(buffer, key, folder);
      return fileUrl;
    } catch (error) {
